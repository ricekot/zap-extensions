...
public class WebUiServer {
    ...
    private final WebUiRenderer renderer;

    private ExtensionWebUi extension;
    private ExtensionNetwork extensionNetwork;
    private Server server;

    public WebUiServer(ExtensionWebUi extension, ExtensionNetwork extensionNetwork) {
    ...
    private Server getServer() {
        if (server == null) {
            this.server = extensionNetwork.createHttpServer(new WebUiRouter());
        }
        return server;
    }
    ...
    public static String getDefaultResponseHeader(
            String responseStatus, String contentType, int contentLength, boolean addCacheHeader) {
        ...
        // connect-src 'self'; "
        //                        + "child-src 'self'; img-src 'self' data:; font-src 'self' data:;
        // style-src 'self'\r\n");
        ...
        if (addCacheHeader) {
            sb.append("Cache-Control: max-age=86400\r\n");
        } else {
    ...
    public void redirect(String url, HttpMessage msg) {
        try {
            msg.setResponseHeader(getDefaultResponseHeader(STATUS_REDIRECT, "text/html", 0, false));
    ...
    private static boolean isApiRequest(HttpMessage msg) {
    ...
    private static boolean isHypermediaRequest(HttpMessage msg) {
        String path = msg.getRequestHeader().getURI().getEscapedPath();
    ...
    private class WebUiRouter implements HttpMessageHandler {
        // TODO: Move this out into a new class?
        @Override
        public void handleMessage(HttpMessageHandlerContext ctx, HttpMessage msg) {
            ...
            try {
                ...
                if (isHypermediaRequest(msg)) {
                    renderer.handleMessage(msg);
                    return;
                }
                String path = msg.getRequestHeader().getURI().getEscapedPath();
                File file = new File(ExtensionWebUi.getBaseDirectory(), path);
                if (file.isDirectory()) {
                    file = new File(file, INDEX_PAGE);
                }
                if (!file.exists()) {
                    LOGGER.debug("File does not exist: {}", file.getAbsolutePath());
                    file = new File(ExtensionWebUi.getBaseDirectory(), ERROR_404_PAGE);
                }
