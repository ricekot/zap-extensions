...
public class ActiveScriptSynchronizer {
    ...
    private final Map<Integer, ActiveScriptScanRule> scriptIdToScanRuleMap = new HashMap<>();
    ...
    public void scriptAdded(ScriptWrapper script) {
        try {
            ...
            // Check if an active script with this ID already exists
            ActiveScriptScanRule scanRule = scriptIdToScanRuleMap.get(metadata.getId());
            if (scanRule != null) {
                if (scanRule.getScript() == script) {
                    scanRule.setMetadata(metadataProvider.getMetadata());
                } else {
                    LOGGER.error(
                            "An active script with the ID ({}) already exists: \"{}\"",
                            metadata.getId(),
    ...
    public void scriptRemoved(ScriptWrapper script) {
        try {
            ...
            ActiveScriptScanRule scanRule =
                    scriptIdToScanRuleMap.get(metadataProvider.getMetadata().getId());
            if (scanRule == null) {
                return;
            }
            ...
            scriptIdToScanRuleMap.remove(metadataProvider.getMetadata().getId());
        } catch (Exception e) {
